...
def test_add_custom_term():
    ...
    assert len(added) == 3
    added = dataset.add_custom_term('more awesomeness', ['9593960', '11114477', 12077008])
    assert len(added) == 1
    assert dataset.get_studies(features='more awesomeness') == [12077008]
    assert len(dataset.get_studies(expression='such awesomeness')) == 3
    features = dataset.get_feature_names()
    assert 'such awesomeness' in features
    assert 'more awesomeness' in features
